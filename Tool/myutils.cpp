...
size_t MyUtils::qstringToCharArray(char* dest, size_t destSize, const QString& src)
{
    if (dest == nullptr || destSize == 0)
        return 0;

    QByteArray utf8 = src.toUtf8();
    size_t copyLen = std::min(static_cast<size_t>(utf8.size()), destSize - 1);
    if (copyLen > 0)
        std::memcpy(dest, utf8.constData(), copyLen);
    dest[copyLen] = '\0';  // 确保字符串终止
    return copyLen;
}