...
void SocketLink::recv_data()
{
    ...
    if(head.type==SERVICE_TYPE::PATIENT_LOGIN){
        PATIENT_LOGIN_RESP resp;
        memcpy(&resp,recv_data.data(),sizeof(PATIENT_LOGIN_RESP));
        qDebug()<<"登录结果:"<<resp.result<<"id:"<<resp.id<<"name:"<<resp.name;
        CData::m_id=resp.id;
        CData::m_name=resp.name;
        if(resp.result){
            emit login_success();
        }
    }
    else if(head.type==SERVICE_TYPE::PATIENT_GET_DOCTOR_INFO){

        GetDoctorInfoTask *task=new GetDoctorInfoTask(head.len,recv_data,this);
        task->execute();
